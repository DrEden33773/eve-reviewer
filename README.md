...
`pnpm build` emits unbundled Node.js 24 ESM and declarations into each public package's ignored `dist/` directory. The core package exposes one root entry. The Adam extension exposes one root entry with named `activate()` and exact compatibility with `@adam-agent/extension-api@0.5.0` and `@eve-reviewer/core@0.3.0`.
