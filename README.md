...
- its trusted Adam extension Adapter preserves `eve-reviewer.review@1` and `eve-reviewer.local-worktree-review@1`, maps Adam's exact generic project-change snapshot to an Eve-owned local-worktree request inside `execute()`, calls Adam's fixed Biome capability and one Host-owned `adam.managed-review@1` request with immutable evidence and a short instruction, composes both required outcomes, preserves validated deterministic partial evidence on distinct typed managed-review failures, publishes the canonical Eve result artifact, creates an immutable operation record, returns only bounded terminal references and reconciles either result read-only after a Host restart.

The source tree defines `@eve-reviewer/core@0.4.0` with the managed-review receipt contract and independently versioned `@eve-reviewer/adam-extension@0.6.0`; the Adapter depends exactly on published core `0.4.0` and declares exact peer compatibility with `@adam-agent/extension-api@0.6.0`. Source versions are not public-release evidence: supported releases on the [`@eve-reviewer/core`](https://www.npmjs.com/package/@eve-reviewer/core) and [`@eve-reviewer/adam-extension`](https://www.npmjs.com/package/@eve-reviewer/adam-extension) package pages require staged approval, provenance, fresh exact-registry installation and real Adam Host conformance. Deprecated, unsupported `0.0.0-bootstrap.0` prereleases remain identified by `bootstrap`.
...
`pnpm build` emits unbundled Node.js 24 ESM and declarations into each public package's ignored `dist/` directory. The core package exposes one root entry. The Adam extension exposes one root entry with named `activate()` and exact compatibility with `@adam-agent/extension-api@0.6.0` and `@eve-reviewer/core@0.4.0`.
