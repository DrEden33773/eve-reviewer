...
The source tree currently contains the coordinated `@eve-reviewer/core@0.0.0-bootstrap.0` and `@eve-reviewer/adam-extension@0.0.0-bootstrap.0` identity-bootstrap artifacts. They have provenance disabled and are not supported releases. Any bootstrap publication must explicitly pass `--tag bootstrap --provenance=false`; npm otherwise defaults to `latest`. Registry publication remains a separate Owner-approved action, and source availability does not imply that either package is already published.
...
`pnpm build` emits unbundled Node.js 24 ESM and declarations into each public package's ignored `dist/` directory. The core package exposes one root entry. The Adam extension exposes one root entry with named `activate()` and exact compatibility with `@adam-agent/extension-api@0.1.0` and the matching `@eve-reviewer/core@0.0.0-bootstrap.0` bootstrap artifact.
